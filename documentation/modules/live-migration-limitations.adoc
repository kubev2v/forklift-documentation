...
* Live migration is available only for migrations between {virt} clusters. It is not available for any other source provider, whether the provider is supported by {project-first} or not. 
...
* Live migration does not migration resources unrelated to VMs, such as services, routes, or other application components, that may be necessary for application availability after a migration.
