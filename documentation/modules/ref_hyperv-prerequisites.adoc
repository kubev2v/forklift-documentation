...
[role="_abstract"]
To migrate from Microsoft Hyper-V to {virt}, verify that your environment meets the requirements and configure host connectivity and storage access. For general software requirements, see link:{mtv-plan}assembly_software-requirements-for-migration_mtv[Software requirements for migration].
...
{project-short} supports Generation 1 and Generation 2 VMs and automatically detects VHD and VHDX disk formats by file extension.
...
* You must use a compatible version of Windows Server with Hyper-V. For more information, see xref:ref_compatibility-guidelines_{context}[Software compatibility guidelines].
* You must configure Windows Remote Management (WinRM) on the Hyper-V host for HTTPS communication.
* A Server Message Block (SMB) share must be available and accessible from both the Hyper-V host and the {ocp-short} cluster.
...
* The service account must be a member of the Hyper-V Administrators group and have read access to the SMB share, for example, Local Administrator.
