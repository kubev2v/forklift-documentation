...
[role="_abstract"]
Verify that your Microsoft Hyper-V environment meets the requirements for migrating virtual machines to {virt}. You must configure WinRM for host connectivity, expose VM disk files on an SMB share, and ensure network access between the Hyper-V host and the {ocp-short} cluster.
