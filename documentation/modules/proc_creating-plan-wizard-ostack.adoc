...
[id="proc_creating-plan-wizard-ostack_{context}"]
...
* Have an {osp} source provider and an {virt} destination provider. For more information, see xref:proc_adding-source-provider_{context}[Adding an OpenStack source provider] or xref:proc_adding-virt-destination-provider_dest_{context}[Adding {a-virt} destination provider].
