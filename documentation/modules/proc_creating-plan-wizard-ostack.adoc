...
* Have an {osp} source provider and an {virt} destination provider. For more information, see xref:proc_adding-source-provider_ostack[Adding an OpenStack source provider] or xref:proc_adding-source-provider_dest_ostack[Adding {a-virt} destination provider].
