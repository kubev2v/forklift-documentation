...
?? ANDY: The comments are for later when I break this note into two, one for OpenStack and for RHV
// ifdef::ostack[]
[NOTE]
====
When you migrate from OpenStack, or when you run a cold migration from {rhv-full} to the OCP cluster that {project-short} is deployed on, the migration allocates persistent volumes without CDI. In these cases, you might need to adjust the file system overhead.
...
//[NOTE]
//====
//When you run a cold migration from {rhv-full} to the OCP cluster that {project-short} is deployed on, the migration allocates persistent volumes without CDI. In these cases, you might need to adjust the file system overhead.
