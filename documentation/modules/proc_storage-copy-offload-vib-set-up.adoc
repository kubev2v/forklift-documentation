...
= Setting up a VIB storage copy offload

[role="_abstract"]
You can set up the vSphere Installation on Bundle (VIB) cloning method to run storage copy offload migrations. This is the default method for running storage copy offload migrations.
...
* SSH private key for ESXi authentication. This can be the the same key as used for the SSH clone method, if you use both.
* Optional: vSphere credentials. These allow you to auto-discovery ESXi hosts.

.Procedure

. Configure the ESXi clone method in your `Provider` CR by setting `settings:exsiCloneMethod` to `"vib"`.  
