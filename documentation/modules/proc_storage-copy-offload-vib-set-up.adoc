...
. Configure the VIB clone method in your `Provider` CR by setting `settings:esxiCloneMethod` to `"vib"`.
+
.Example Provider CR
...
$ podman run -it --rm \
  ...
  $({oc} get deployment forklift-volume-populator-controller  -n {namespace} -o jsonpath='{.spec.template.spec.containers[0].env[?(@.name == "VSPHERE_XCOPY_VOLUME_POPULATOR_IMAGE")].value}') \
...
[NOTE]
=====
For alternative VIB installation methods using Ansible, see link:https://github.com/kubev2v/forklift/blob/main/cmd/vsphere-copy-offload-populator/vmkfstools-wrapper/README.md[Esxcli plugin that wraps vmkfstools].
=====
