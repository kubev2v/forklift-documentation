...
Windows Server 2012 R2 VMs do not boot after migration from {vmw}::
The current `virtio-win` package does not include VirtIO drivers for Windows Server 2012 or Windows Server 2012 R2. As a consequence, when you migrate a Windows Server 2012 R2 virtual machine (VM) from {vmw} vSphere, {project-short} cannot inject the required storage and network drivers during conversion. The VM does not boot correctly on {virt} and might enter Windows Recovery Environment or a login loop.
+
Currently, there is no workaround for this issue.
+
link:https://redhat.atlassian.net/browse/MTV-6329[MTV-6329]
