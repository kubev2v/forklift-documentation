...
[role="_abstract"]
Cold migration is for VMs that are shut down and works with {vmw} vSphere, {rhv-full}, {osp}, Open Virtual Appliances (OVAs) created by {vmw} vSphere, Microsoft Hyper-V, and remote {virt} clusters. Warm migration is for running VMs and works with {vmw} vSphere and {rhv-full}.
