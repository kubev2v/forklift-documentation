...
[role="_abstract"]
To ensure that your custom validation rules persist across pod restarts, scaling events, and {project-first} upgrades, deploy the rules by using a *ConfigMap* Custom Resource (CR) and reference the rules in the `forklift-controller` CR.
...
. Create a `ConfigMap`, for example `forklift-validation-config`:
+
Example:
+
