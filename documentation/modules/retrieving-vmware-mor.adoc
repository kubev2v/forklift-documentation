...
[id="retrieving-vmware-mor_{context}"]
= Retrieving a VMware vSphere MOR

When you migrate VMs with a VMware vSphere source provider using {project-first} from the CLI, you need to know the managed object reference (MOR) of certain entities in vSphere, such as datastores, networks, and VMs.

You can retrieve the MOR of one or more vSphere entities from the Inventory service. You can then use each MOR as a reference for retrieving the MOR of another entity.
...
. Retrieve the MOR of a VMware vSphere provider:
+
[source,terminal]
----
...
In this example, the MOR of the datastore `v2v_general_porpuse_ISCSI_DC` is `datastore-11` and the MOR of the datastore `f01-h27-640-SSD_2` is `datastore-730`.
.
