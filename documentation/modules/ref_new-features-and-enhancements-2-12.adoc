...
Support for the `pvcNameTemplate` parameter in {virt} to {virt} migrations::
{project-short} supports the `pvcNameTemplate` parameter for {virt} to {virt} migrations. This enhancement is implemented to enable the customization of Persistent Volume Claim (PVC) names during the migration process. As a result, you can specify custom names for PVCs when migrating virtual machines (VMs) between {virt} environments.
+
