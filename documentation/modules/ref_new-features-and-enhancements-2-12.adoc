...
* You must install the NetApp Shift toolkit add-on on the {virt} cluster alongside the Trident Operator.
* Source disks must reside on an ONTAP NFS datastore. You cannot use basic VMFS datastores.
* You must annotate the applicable storage classes within the UI to identify them as NetApp Shift toolkit-capable.
* Source VMs cannot have active snapshots. You must flatten all disks before you initiate the migration.
* You can perform only cold migrations from {vmw} sources. 
