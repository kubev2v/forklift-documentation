...
== {project-short} 2.12.2

CSI volume import is available for copy-offload migrations (Developer Preview)::   
{project-short} introduces a new copy-offload method that uses Container Storage Interface (CSI) driver volume import capabilities. This method migrates VMware vSphere Virtual Volumes (vVols) and Raw Device Mapping (RDM) disks. Instead of running a dedicated populator pod per disk, {project-short} creates a persistent volume claim (PVC) with vendor-specific import annotations. The CSI driver handles volume provisioning from the existing source volume. As a result, the CSI driver manages the volume lifecycle. This enhancement makes migrations safer, reduces configuration requirements, and improves reliability.
+
