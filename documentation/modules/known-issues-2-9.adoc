...
.Source storage not mapped in the {project-short} user interface for {ocp-short} to {ocp-short} migration
...
When you migrate a virtual machine (VM) with trusted platform module (TPM) from {ocp-short} to {ocp-short}, the VM uses a new persistent volume claim (PVC) that is created for the persistent-state TPM device. link:https://issues.redhat.com/browse/MTV-2838[(MTV-2838)]
