...
The Node.js Package Manager (npm) IP package has a vulnerability that allows an attacker to arbitrarily run code and obtain sensitive information. The attack induces a Server-Side Request Forgery (SSRF) attack by using the `isPublic()` function to obtain access to resources that are normally protected.
