...
.Migration using OpenStack moves to being a fully supported feature
...
In {project-short} {project-version}, you can also use Red Hat OpenShift Virtualization provider as a source provider. You can migrate VMs from the cluster that MTV is deployed on to another cluster, or from a remote cluster to the cluster that {project-short} is deployed on.
