...
An outdated `ovirtvolumepopulator` in the namespace the is left over from an earlier failed migration causes a new plan of the same VM to stop when it transitions to `CopyDisks` phase. The plan remains in that phase indefinitely. link:https://issues.redhat.com/browse/MTV-929[(MTV-929)]

.Unclear error message when Forklift fails to build/create a PVC

The migration fails to build a PVC when the destination storage class does not have a configured storage profile. The error logs do not provide enough information to identify the reason for failure.
...
.UEFI is imported as BIOS

When migrating OVA of VMware VM ith EFI firmware, migrated VM is created with BIOS firmware instead of UEFI, causing a boot failure for the VM. Manually changing the BIOS type to UEFI avoids this problem. link:https://issues.redhat.com/browse/MTV-666[MTV-666]
