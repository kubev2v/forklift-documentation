...
A vulnerability in Node.js Package Manager (npm) IP package allowed an attacker to run code and obtain sensitive information. This issue has been resolved in {project-short} 2.5.6 by upgrading the package `node-ip` to version 2.0.1
...
OpenShift Web Console removed the `match` property in earlier versions. This removal impacted pages that passed parameters through the URL, thereby preventing users from creating a migration plan in {project-short} Web Console. This issue has been resolved in {project-short} 2.5.6. link:https://issues.redhat.com/browse/MTV-952[(MTV-952)]
