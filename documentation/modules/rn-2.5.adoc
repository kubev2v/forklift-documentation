...
{project-short} 2.5.6 can be deployed on {ocp-name} 4.15 clusters. The operator includes a temporary fix for link:https://issues.redhat.com/browse/OCPBUGS-30095[Forklift operator fails on {ocp-name} 4.15]. The console plugin includes a fix for the `match` property that was removed in {ocp-name} Web Console. link:https://issues.redhat.com/browse/MTV-952[(MTV-952)]
...
In {project-short} 2.5.3, deployment on {ocp-name} Kubernetes Engine (OKE) has been enabled. For more information, see link:https://docs.openshift.com/container-platform/4.14/welcome/oke_about.html[About {ocp-name} Kubernetes Engine]. link:https://issues.redhat.com/browse/MTV-803[(MTV-803)]
...
A vulnerability found in the Node.js Package Manager (npm) IP Package can allow an attacker to obtain sensitive information and obtain access to normally inaccessible resources.
