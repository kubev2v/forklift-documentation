...
In {project-short} 2.5.3, deployment on OpenShift Kubernetes Engine (OKE) has been enabled. For more information, see link:https://docs.openshift.com/container-platform/4.14/welcome/oke_about.html[About OpenShift Kubernetes Engine].

link:https://issues.redhat.com/browse/MTV-803[(MTV-803)]
...
----
<1> The percentage of overhead. If this label is not added, the default value of 10% is used. This setting is valid only if the `storageclass` is `filesystem`.
====

link:https://issues.redhat.com/browse/MTV-699[(MTV-699)]
