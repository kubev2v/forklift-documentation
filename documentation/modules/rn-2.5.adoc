...
The Node.js Package Manager (npm) IP package has a vulnerability that allows an attacker to arbitrarily run code and obtain sensitive information. This vulnerability can allow a Server-Side Request Forgery (SSRF) attack, by using the `isPublic()` function to obtain access to resources that are normally protected.
