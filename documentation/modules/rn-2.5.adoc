...
With VMware vSphere source providers, Enterprise Master Secret (EMS) enforcement is disabled for migrations to enable migrations from versions of vSphere that are supported by {project-short} but do not meet the 2023 FIPS requirements.
...
A vulnerability in Node.js Package Manager (npm) IP package that allowed an attacker to arbitrarily run code and obtain sensitive information. The operator was fixed on OpenShift 4.15, enabling MTV 2.5.6 to deploy the `match` property in the UI.

This issue has been resolved in MTV 2.5.6 by upgrading the package `node-ip` to version 2.0.1

For more information, see link:https://bugzilla.redhat.com/show_bug.cgi?id=2265161[CVE-2023-42282]

.Missing match property in Console 4.15

OpenShift Console removed the `match` property in earlier versions. This impacted pages that passed parameters through the URL, preventing users from creating a migration plan in MTV Console. link:https://issues.redhat.com/browse/MTV-952[(MTV-952)]

This issue has benn resolved in MTV 2.5.6.
...
During the migration from Red Hat Virtualization (RHV), direct Logical Units (LUNs) are detached from the source virtual machines and attached to the target virtual machines. Note that this mechanism does not work yet for Fibre Channel. link:https://issues.redhat.com/browse/MTV-329[(MTV-329)]
...
Warm migration from {rhv-short} fails if a snapshot operation is triggered and running on the source VM at the same the migration is scheduled. The migration does not wait for the snapshot operation to end. link:https://issues.redhat.com/browse/MTV-456[(MTV-456)]
...
.`--root first` virt-v2v additional argument to support multi-boot 

The virt-v2v arguments include `–root first`, which mitigates an issue with multi-boot VMs where the pod fails. Adding this argument back to the entrypoint will resolve the issue. Link:https://issues.redhat.com/browse/MTV-968[(MTV-968)]
