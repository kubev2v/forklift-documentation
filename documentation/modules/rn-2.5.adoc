...
Warm migration from {rhv-short} fails if a snapshot operation is triggered on the source VM. The migration fails if it is scheduled to beging when a snapshot operation is running. The migration does not wait for the snapshot operation to end. link:https://issues.redhat.com/browse/MTV-456[(MTV-456)]
