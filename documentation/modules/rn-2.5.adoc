...
When adding an OVA provider, the error message `ConnectionTestFailed` appears, although the provider is created successfully. After a few seconds, this message disappears. link:https://issues.redhat.com/browse/MTV-671[MTV-671]
...
This issue is resolved in {project-short} {project-version}, warm migration does not fail when an operation that locks the VM is performed in {rhv-short}. The migration does not fail, but starts when the VM is unlocked. link:https://issues.redhat.com/browse/MTV-686[MTV-686] 
