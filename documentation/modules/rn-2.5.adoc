...
Enterprise Master Secret (EMS) enforcement is disabled for migrations with VMware vSphere source providers to enable migrations from versions of vSphere that are supported by {project-short} but do not comply with the 2023 FIPS requirements.
...
.`isPublic()` function executes code arbitrarily

The Node.js Package Manager (npm) IP package has a vulnerability that allows an attacker to arbitrarily run code and obtain sensitive information. This failure to identify certain private IP addresses in uncommon formats as `private` can lead to security issues, such as Server-Side Request Forgery (SSRF), if `isPublic()` is used to protect sensitive code paths when passed user input.
...
OpenShift Console removed the `match` property in earlier versions. link:https://issues.redhat.com/browse/MTV-952[(MTV-952)]
...
.Migration uses OVA files created by VMware vSphere
...
Warm migration from {rhv-short} fails if the user performs a snapshot operation on the source VM. The migration does not wait for a snapshot operation on the source VM to finish when it is scheduled at the same time as the migration. link:https://issues.redhat.com/browse/MTV-456[(MTV-456)]
