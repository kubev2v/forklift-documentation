...
.Setting the VddkInitImage URL is part of the procedure of adding provider.

In the web console, you enter the VddkInitImage URL when adding a VMware provider. Alternatively, from the CLI, you add the VddkInitImage URL to the `Provider` CR for VMware migrations.

.The `StorageProfile` resourse needs to be updated for a non-provisioner storage class

You must update the `StorageProfile` resourse with `accessModes` and `volumeMode` for non-provisioner storage classes such as NFS. The documentation includes a link to the relevant procedure.
