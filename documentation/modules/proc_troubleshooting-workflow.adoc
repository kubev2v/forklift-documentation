...
// SME CHECK: Review the flow of this troubleshooting workflow in the full assembly context
// after adding Lightspeed AI as step 2. Verify that the xref to proc_using-lightspeed-troubleshooting
// resolves correctly and that the workflow progression makes sense.
...
+
If you have Red Hat OpenShift Lightspeed configured with {project-short}, you can ask questions in natural language to get plain-language explanations of migration failures, recommended solutions, and guidance for resolving common issues.
+
For example: "Why did the migration for VM 'ProdDB-01' fail?"
+
See xref:proc_using-lightspeed-troubleshooting_{context}[Using Red Hat OpenShift Lightspeed to troubleshoot migration issues] for detailed instructions.
...
+
The `must-gather` tool collects comprehensive diagnostic information about your cluster. Red Hat OpenShift Lightspeed analyzes the same types of data that `must-gather` collects, but `must-gather` provides a complete archive for Red Hat Support analysis.
+
...
* xref:collected-logs-cr-info_{context}[Logs and custom resources for troubleshooting]
* xref:error-messages_{context}[Error messages]
