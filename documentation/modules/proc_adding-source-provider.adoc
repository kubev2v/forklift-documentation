...
[role="_abstract"]
You can add a Microsoft Hyper-V source provider by using the {ocp} web console. Hyper-V virtual machines are exported to an SMB shared directory for migration.
...
* *Provider resource name*: Name of the source provider.
* *URL*: Path to the SMB shared directory containing the Hyper-V exported VMs. For example, `//192.168.122.115/VMShare` or `\\192.168.122.115\VMShare`.
+
The SMB shared directory must be accessible from both the Hyper-V host and the {ocp-short} cluster.
