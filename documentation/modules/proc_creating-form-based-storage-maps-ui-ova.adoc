...
* Have an Open Virtual Appliance (OVA) source provider and {a-virt} destination provider. For more information, see xref:proc_adding-source-provider_ova[Adding an Open Virtual Appliance (OVA) source provider] or xref:proc_adding-virt-destination-provider_dest_ova[Adding {a-virt} destination provider].
