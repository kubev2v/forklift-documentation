...
** {rhv-short} admin permissions. These permissions allows you to migrate any virtual machine in the system.
** `DiskCreator` and `UserVmManager` permissions: These permissions have to be granted by an `admin` user to you individually per each individual virtual machine that you want to migrate.
