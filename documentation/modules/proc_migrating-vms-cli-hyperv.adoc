...
`<smb_server_path>`::
Specifies the SMB share path in the format `//server/share` or `\\server\share`, for example, `//192.168.122.115/VMShare`.

`<smb_username>`::
Specifies the username for accessing the SMB share, for example, `Administrator` or `DOMAIN\username`.

`<smb_password>`::
Specifies the password for accessing the SMB share.
...
`<source_network_id>`::
Specifies the Hyper-V network adapter (virtual switch) name or ID. The name is the virtual switch friendly name. The ID is the switch GUID in 8-4-4-4-12 hexadecimal format.
...
`<source_storage_id>`::
Specifies the Hyper-V storage identifier, such as the file path to the virtual hard drive (for example, `C:\VHDs\disk.vhdx`), a volume GUID path, or a storage pool name.
...
`<source_vm1>`::
Specifies the Hyper-V VM identifier.
