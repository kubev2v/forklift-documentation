...
+
Raw copy mode is intended for specific scenarios where the guest is already KVM-compatible. It requires a valid pre-populated source or specific disk configuration. Otherwise, the migration can result in empty PVCs rather than a raw data copy of the disk.
+
...
skipGuestConversion is intended for scenarios where the guest is *already KVM-compatible* or conversion is not feasible.
It *requires a valid pre-populated source and appropriate disk configuration*.
Using it with an incompatible source or misconfigured disks can result in *empty PVCs* rather than a raw copy of the disk data.
