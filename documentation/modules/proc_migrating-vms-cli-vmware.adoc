...
`<user>`::
Specifies the user of the vCenter, ESX or ESXi.

`<password>`::
Specifies the password of the user of the vCenter, ESX or ESXi.
...
`<api_end_point>`::
Specifies the API endpoint URL of the vCenter, the ESX or the ESXi, for example, `https://<vCenter_host>/sdk`.
...
`targetAffinity`::
Specifies a VM target affinity rule that is entered in the lines following this label. This is an optional field.
+
...
`targetLabels`::
Specifies organizational or operational labels to migrated VMs for identification and management. This is an optional field.

`targetNodeSelector`::
Specifies the key-value pairs that must be matched for VMs to be scheduled on nodes. This is an optional field.

`convertorLabels`::
Cold migrations only: Specifies organizational or operational labels for the `virt-v2v` convertor pods (importer pods) for identification and management. This is an optional field.
+
[IMPORTANT]
====
