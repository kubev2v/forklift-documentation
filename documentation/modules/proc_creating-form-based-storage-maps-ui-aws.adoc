...
[IMPORTANT]
====
Storage mapping for {aws} {ec2} migration is limited to the same storage type. You must map EBS volumes to storage classes that use the {aws} EBS CSI driver.
====
...
// SME REVIEW: Are there any additional AWS-specific storage prerequisites?
...
+
// SME REVIEW: Can you provide:
// - Screenshot of the storage map creation form for AWS EC2
// - List of supported EBS volume types (e.g., gp2, gp3, io1, io2, st1, sc1)
// - Any volume types that are not supported
+
* *Target storage*: Select a storage class that uses the EBS CSI driver from the list.
+
// SME REVIEW: What does "same storage only" limitation mean exactly?
// - Can you provide examples of valid storage mappings?
// - Can you provide examples of invalid storage mappings?
// - Are there recommended storage class configurations?
