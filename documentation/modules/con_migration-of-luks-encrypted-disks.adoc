...
If you select an existing `Secret` manifest, the secret must meet these requirements:

* The `Secret` manifest must have the `Opaque` type.
* The secret must reside in the migration plan project.
* Each data entry in the secret must represent one passphrase.
* You can use any key name in the `data` or `stringData` fields.
