...
* *Technology Preview:* In {project-short} 2.10.0, the Storage Offload Copy feature accelerates disk transfer in the storage array by bypassing the network. This update improves copy speed with the following options:
** Real-time disk transfer for running VMs. 
** Snapshot copies from a running VM.
** Copies of delta changes from a snapshot after the storage offload copy, minimizing the requirement for VM shutdowns. link:https://issues.redhat.com/browse/MTV-2637[(MTV-2637)]  
