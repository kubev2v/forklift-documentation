...
* Have an {osp} source provider and {a-virt} destination provider. For more information, see xref:proc_adding-source-provider_ostack[Adding an {osp} source provider] or xref:proc_adding-source-provider_dest_ostack[Adding {a-virt} destination provider].
