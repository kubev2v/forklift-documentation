...
You can add a `PreHook` or a `PostHook` Hook CR when you migrate an application from the command line by using the {project-full} API. A `PreHook` runs before a migration, a `PostHook`, after.

[NOTE]
====
You can retrieve additional information stored in secrets or a `configMap`s by using `k8s` modules.
====
