...
= Migrating an application by using the {project-short} API

You can migrate an application from the command line by using the {project-full} API.
...
[discrete]
== Default hook image
The default hook image is `quay.io/konveyor/hook-runner:latest`. It is based on the Ansible Runner image with the addition of `python-openshift` to provide Ansible Kubernetes resources and a recent `oc` binary.
// The most recent?

[discrete]
== Hook execution
When an Ansible playbook is provided as part of a migration hook, it is mounted into the hook container as a `ConfigMap`. The hook container is run as a job on the desired cluster, using the default `ServiceAccount` in the `konveyor-forklift` namespace.

== Adding a hook to a Plan
Hooks can be specified per VM and may be run as _PreHooks_ or _PostHooks_. PreHooks are hooks run before , pre or post migration?). When adding a hook you must specify the namespace where the hook CR is located, the name of the hook, and specify whether it should be run as a PreHook or PostHook.
