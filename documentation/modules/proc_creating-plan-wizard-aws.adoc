...
// Is this correct (EBS volumes map to EBS-backed storage classes only)?
...
[role="_abstract"]
You can migrate {aws} {ec2} instances with the {project-full} plan creation wizard.
...
* {project-short} maps EBS volumes to EBS-backed storage classes only.
...
* {ec2} instances you want to migrate must be stopped.
...
[IMPORTANT]
====
For {aws} {ec2} migrations, you must map EBS volumes to storage classes that use the {aws} EBS CSI driver.
====
