...
A new populator implementation under cmd/vsphere-xcopy-volume-populator is a cli program that runs in a container that is responsible to perform XCOPY to efficiently copy data from a VMDK to the target PVC.

{project-first} 2.10.0 leverages this command as the basis for storage copy offload, which clones your VMs' data to the storage hardware instead of transmitting it between {project-short} and {virt}. This improved migration saves both time an resources.

You implement storage copy offload by configuring the storage map in your migration plan to point to your storage array instead of the network you usually use for migration. When you start the migration plan, {project-short} migrates your VMs by copying them to the storage array you choose and using `XCOPY` to copy them directly to {virt}, instead of transmitting the contents of your VMs to {virt}.
...
[IMPORTANT]
====
The storage arrays must be the ones specified above. Otherwise, `XCOPY` will perform a network copy on the ESXi. Although a network copy on the ESXi is usually considerably faster than a standard migration using an VDDK image, it is not as quick as a properly configured storage copy offload migration.
====
