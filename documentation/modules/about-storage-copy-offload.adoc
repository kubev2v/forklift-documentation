...
{vmw}'s vSphere Storage APIs-Array Integration (VAAI) includes a command named `vmkfstools`. This command sendq the `XCOPY` command, which is part of the SCSI protocol. The `XCOPY` commandS lets you copy data inside a SAN more efficiently than copying the data over a network.
...
. Instead of sending the data, {project-short} instructs the storage array in which the Vsphere Virtual Machine File System (VMFS) datastore holds the source VMs to perform a direct copy from the source storage to the target array {virt} in the correct storage class.
+
