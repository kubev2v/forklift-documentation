...
{vmw}'s vSphere Storage APIs-Array Integration (VAAI) includes a command named `XCOPY`. This command lets you copy data inside a SAN more efficiently than copying the data over a network.

{product-first} 2.10.0 leverages this command as the basis for storage copy offload, allowing you to delegate cloning your VMs' data to the storage hardware instead having it to transit it between {project-short} and {virt}. You do this by configuring the storage map in your migration plan to point to your storage array instead of the network you usually use for migration. When you start the migration plan, {project-short} migrates your VMs by copying them to the storage array you choose and using `XCOPY` to copy them directly to {virt}, instead of transmitting the contents of your VMs to {virt}. 

The storage copy offload feature has some unique configuration prerequisites, which are discussed in TBD. Once you configure your system, you can migrate plans using storage copy offload by using either the {project-short} UI or its CLI. Instructions for using storage offload have been integrated into the procedures for migrating {vmw} VMs for both the Ui and CLI. 

[NOTE]
====
If you perform storage-offload migrations from a single ESXi host, for example, vSphere 7.0.3, the migration might fail if more than two migrations are active concurrently.

This is because the underlying VMware utility, `vmkfstools`, strictly enforces a limit of two active copy operations per ESXi host. If the internal scheduling function of {project-short} incorrectly allows the start of more than two simultaneous offload operations (`populate-pods`), this limit is exceeded, causing the migrations to fail.

To prevent this issue, ensure that you limit the number of concurrent storage-offload migrations originating from a single ESXi host to a maximum of two.
====

Storage copy offload is available as a Technology Preview feature for {project-first} 2.10.0 for cold migration and as a Developer Preview feature for warm migration. It is not available for shared disks for either cold or warm migrations.
...
[id="storage-copy-offload-vendors_{context}"]
== Supported vendors

The table that follows lists the vendors whose storage products are supported by storage copy offload and the product name to use when migrating by using the CLI:

[cols="1,1",options="header"]
.Supported storage vendors  
|===
|Vendor
|Value of storageVendorProduct label
