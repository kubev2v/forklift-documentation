...
+
*Workaround:* Manually delete the leftover `ova-store-pvc-*` PVCs after migration by using the `oc delete pvc <pvc_name>` command. This action removes the excess resources, but data loss might occur if the PVCs contain important data. 
+
...
+
*Workaround:* Check the write permission of the OVA file before initiating the upload to complete the upload process without permissions errors.
+ 
