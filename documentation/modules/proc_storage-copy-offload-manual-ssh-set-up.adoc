...
= Setting up storage copy offload using manually generated SSH keys
...
---- 
// Should this be <namespace> or as is?
...
[source,terminal]
----
echo 'command="python /vmfs/volumes/<datastore_name>/secure-vmkfstools-wrapper.py",no-port-forwarding,no-agent-forwarding,no-X11-forwarding '$(cat esxi_public_key.pub) > restricted_key.pub
----
