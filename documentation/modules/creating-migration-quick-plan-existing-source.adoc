...
+
{project-short} uses the following format for disk location: `/dev/sd<disk_number><disk_partition>`. For example, if the second disk is the root device and the operating system is on the disk's second partition, the format would be: `/dev/sdb2`. After you enter the boot device, click *Save*.
+
