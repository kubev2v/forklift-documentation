...
The main function of `virt-v2v` is to convert the disk image of a VM into a format compatible with {virt}.

This program is described in detail in xref:virt-v2v-mtv_{context}[How {project-short} uses the virt-v2v tool].
...
* For information about configuring raw copy mode by using the {project-short} user interface, see link:{mtv-plan}assembly_planning-migration-vmware_mtv#proc_configuring-plan-settings-vmware_vmware[Configuring VMware migration plan settings] in _Planning your migration to Red Hat {virt}_.
* For information about configuring raw copy mode by using the command-line interface, see the `skipGuestConversion` and `useCompatibilityMode` parameters in xref:proc_migrating-vms-cli-vmware_vmware[Running a VMware vSphere migration from the command-line].
* For information about device compatibility mode for raw copy migrations, see xref:con_device-compatibility-mode-raw-copy-migrations_mtv[Device compatibility mode for raw copy migrations].
