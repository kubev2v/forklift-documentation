...
.Accellerated migration using conversion pod disk transfer
...
A 'Skip certificate validation' option was added to VMware and RHV providers. If checked, the provider's certificate will not be validated and the UI will not ask for specifying a CA certificate.
...
.Cannot schedule migrated VM with multiple disks to more than one storage classes of typw hostpath
...
.Prevent user accounts from being locked due to incorrect credentials
