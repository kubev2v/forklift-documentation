...
[id="proc_selecting-migration-network-for-virt-provider_{context}"]
...
[IMPORTANT]
====
{project-short} supports using UDNs for migrations from all source providers except when the source provider is also {virt} (for OpenShift-to-OpenShift migrations).
====
