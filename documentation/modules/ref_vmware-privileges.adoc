...
[role="_abstract"]
Configure user permissions in {vmw} vSphere to allow {project-first} to access and migrate virtual machines (VMs). These permissions control what {project-first} can read, modify, and execute during the migration process. Setting up the correct privileges ensures successful migrations while maintaining security by granting only the necessary access rights.

The required privileges vary based on migration type (cold or warm) and VM characteristics (encrypted or unencrypted). All migrations require datastore access and session validation. Warm migrations require additional snapshot and guest OS management privileges. Encrypted VMs require cryptographic privileges.

The following {vmw} privileges are required to migrate VMs to {virt}.

== Privileges required for all migrations

The following privileges are required for all VM migrations, regardless of migration type or VM characteristics.
...
== Privileges for warm migrations

The following privileges are required only for warm migrations or when using pre-migration hooks that access the VM.
...
The following provisioning privileges are required for warm migrations to access VM disks and transfer configuration files.
...
The following provisioning privileges may be required depending on your environment and VM configuration, such as when migrating templates or VMs that require customization.

[cols="2", options="header"]
.`Virtual machine.Provisioning` privileges (optional)
|===
|Privilege |Description
...
The following snapshot management privileges are required only for warm migrations.
...
== Privileges for encrypted VMs

The following privileges are required only when migrating encrypted VMs.
