...
[id="proc_mtv-enable-aio-buffer_{context}"]
...
. Ensure that the `forklift-controller` pod in the +{namespace}+ namespace supports the AIO buffer values. Since the pod name prefix is dynamic, check the pod name by running the following command:
+
[source,terminal,subs="attributes+"]
----
{oc} get pods -n {namespace} | grep forklift-controller | awk '{print $1}'
----
+
...
[source,terminal,subs="attributes+"]
----
{oc} get pod forklift-controller-667f57c8f8-qllnx -n {namespace} -o yaml
----
+
