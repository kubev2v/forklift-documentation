...
Optional. Specifies the {aws} access key ID for the target account. Required only for cross-account migrations.
+
`<target_aws_secret_access_key>`::
Optional. Specifies the {aws} secret access key for the target account. Required only for cross-account migrations.
+
[NOTE]
====
