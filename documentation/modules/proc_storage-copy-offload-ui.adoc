...
* It is strongly recommended to delete any pre-existing snapshots before you start a storage copy offload migration. Pre-existing snapshots prevent use of `XCOPY` and force `vmkfstools` to use a slower software-based cloning method. This issue is especially important for performance-sensitive storage copy offload migrations.

[NOTE]
====
Snapshots created by a warm migration do not affect storage copy offload migrations, only pre-existing snapshots cause this issue.
====
