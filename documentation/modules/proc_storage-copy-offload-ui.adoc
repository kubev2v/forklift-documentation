...
| `GOVMOMI_HOSTNAME`
| The hostname or URL of the vSphere API (string)
| Yes
|

| `GOVMOMI_USERNAME`
| The username of the vSphere API (string)
| Yes
|

| `GOVMOMI_PASSWORD`
| The password of the vSphere API (string)
| Yes
|
...
| `STORAGE_ID`
| The storage array serial number (string)
| Yes
|

| `HOSTGROUP_ID_LIST`
| A list of I/O ports and host group IDs, for example, `CL1-A,1:CL2-B,2:CL4-A,1:CL6-A,1`
| Yes
|
|===
+
...
| `STORAGE_HOSTNAME`
| The IP or URL of the host (string). Either enter the management IP for the entire cluster or enter a dedicated storage virtual machine management logical interface (SVM LIF).
| Yes
|
...
| `ONTAP_SVM`
| The storage virtual machine (SVM) to be used in all client interactions. It can be taken from the `trident.netapp.io/v1/TridentBackend.config.ontap_config.svm` resource field.
| Yes
|
|===
+
...
| `STORAGE_HOSTNAME`
| The IP or URL of the host (string)
| Yes
|
...
| The Symmetrix ID of the storage array. It can be taken from the config map under the `powermax` namespace, which the CSI driver uses.
| Yes
|
...
| `POWERFLEX_SYSTEM_ID`
| The system ID of the storage array. It can be taken from `vxflexos-config` from the `vxflexos` namespace or from the `openshift-operators` namespace.
| Yes
|
|===
+
...
.. Create an ownerless storage map by using the procedure in link:{mtv-plan}assembly_planning-migration-vmware_mtv#proc_creating-form-based-storage-maps-ui-vmware_vmware[Creating ownerless storage maps using the form page of the {project-short} UI]. Use the *Offload plugin* named `vSphere XCOPY`.
