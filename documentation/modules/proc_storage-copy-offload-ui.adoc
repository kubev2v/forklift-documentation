...
* It is strongly recommended to delete any pre-existing snapshots before you start a storage copy offload migration, especially performance-sensitive migrations. Pre-existing snapshots prevent use of `XCOPY` and force `vmkfstools` to use a slower software-based cloning method.
