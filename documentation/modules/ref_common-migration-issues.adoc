...
[id="ref_common-migration-issues_{context}"]
...
*Why do Completed migration or validation pods still show PVC mounts?*

For failed migrations, the migration plan status *Incomplete* in the UI, or `failed` in the YAML file, means the migration workflow finished but {project-short} did not remove temporary pods or release PVC mounts. Completed migration and validation pods, such as `conversion` and `importer` pods, continue to mount PVCs until you archive the migration plan. This allows you to investigate what went wrong with the migration. For successful migrations, {project-short} automatically deletes PVC mounts, conversion pods, and populator pods.

*Why are disaster recovery relocations or failovers stuck after MTV migrations?*

For failed migrations, unarchived migration plans can leave PVCs mounted by completed {project-short} pods. Downstream disaster recovery tooling that uses the same storage can be blocked, causing relocation or failover operations to become stuck.
