...
* Have {a-virt} source provider and {a-virt} destination provider. For more information, see xref:proc_adding-source-provider_cnv[Adding {a-virt} source provider] or xref:proc_adding-virt-destination-provider_dest_cnv[Adding {a-virt} destination provider].
