...
[id="proc_creating-plan-wizard-cnv-live_{context}"]
...
+
If they are both {virt} clusters, have someone with `cluster-admin` privileges ensure that the prerequisites for live migration are met. For more information, see xref:ref_cnv-cnv-live-prerequisites_mtv[{virt} live migration prerequisites].
