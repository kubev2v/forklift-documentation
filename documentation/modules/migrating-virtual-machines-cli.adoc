...
. Create a `Secret` CR manifest for the source provider credentials:
+
...
. Create a `Provider` CR manifest for the source provider:
+
[source,yaml,subs="attributes+"]
----
$ cat << EOF | oc apply -f -
...
. Create a `StorageMap` CR manifest to map source and destination storage:
+
[source,yaml,subs="attributes+"]
----
$ cat << EOF | oc apply -f -
...
. Optional: Create a `Hook` CR manifest to run custom code on a VM during the phase specified in the `Plan` CR:
+
[source,yaml,subs="attributes+"]
----
$  cat << EOF | oc apply -f -
...
. Optional, for VMware only: To change the time interval between the CBT snapshots for warm migration, patch the `vm-import-controller-config` config map:
+
[source,terminal]
----
