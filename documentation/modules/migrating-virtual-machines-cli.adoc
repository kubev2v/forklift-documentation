...
* {virt} destination provider only: You must have an {virt} service account token with `cluster-admin` privileges.
...
<4> Specify the vCenter user, the {rhv-short} {manager} user, or the {osp} user. For {ocp-short}, instead of entering a `user`, create a service account and a token, as described in xref:adding-destination-providers[Adding an OpenShift Virtualization destination provider]. This property is not used for OVA.
...
This property is not used for OVA.
...
<2> Specify the Managed Object Reference (MoRef) of the VMware host.
...
<3> Specify the VMware network Managed Object Reference (MoRef), the {rhv-short} network Universal Unique ID (UUID), the {osp} network UUID, or the OVA network UUID.
...
<2> Specify the VMware data storage MoRef, the {rhv-short} storage domain UUID, or the {osp} `volume_type` UUID. For OVA, use `name: Dummy storage for source provider <provider_name>` instead of `id: <source_datastore>`.
<3> {ocp-short} only.
+
[NOTE]
====
...
<9> Specify the VMware VM MoRef, the {rhv-short} VM UUID, the {osp} VM UUID, or the OVA VM UUID.
