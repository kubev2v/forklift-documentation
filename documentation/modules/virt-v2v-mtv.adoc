...
[role="_abstract"]
The {project-first} uses the `virt-v2v` tool to convert the disk image of a VM into a format compatible with {virt}. The tool makes migrations easier because it automatically performs the tasks needed to make your VMs work with {virt}. For example, enabling paravirtualized VirtIO drivers in the converted virtual machine (VM), if possible, and installing the QEMU guest agent.
...
[NOTE]
====
You can also run predefined Ansible hooks before or after a migration using {project-short}. For more information, see xref:adding-hooks-mtv-migration-plan[Adding hooks to an MTV migration plan].
...
[NOTE]
====
For RHEL-based guests, `virt-v2v` attempts to install the guest agent from the Red Hat registry. If the migration is run in a detached environment, the installation program will fail, and you must use hooks, or other automation, to install the guest agent.
====
