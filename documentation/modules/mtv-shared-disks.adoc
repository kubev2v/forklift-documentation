...
In the first figure, the first `Plan` CR migrates VMs 2 and 4, shared disks 1, 2, and 3, and the non-shared disks of VMs 2 and 4. VMs 2 and 4 were chosen because together they connect to all the shared disks. In practice, {project-short} first migrates VM 2 and all disks attached to it, then does the same with VM 4. Choosing VM 2 and VM 4 and only those two ensures that all the shared disks are migrated, but only once.  

The second `Plan` CR migrates VMs 1 and 3 and their non-shared disks. {project-short} is able to reattach all VMs to their disks, including the shared disks. 
...
In this case, in the first `Plan` CR, {project-short} first migrates VM 2, its unshared disks, and shared disks 1 and 2. It then migrates VM 3, its unshared disks, and shared disks 2 and 3. In the second `Plan` CR, {project-short} migrates the other VMs and their unshared disks. 
