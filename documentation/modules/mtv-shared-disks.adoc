...
Shared disks are disks that are attached to multiple VMs and that use the multi-writer option. As a result of these characteristics, shared disks are difficult to migrate. 
...
* If set to 'false`, {project-short} uses the KubeVirt CDI for disk transfer, and skips the shared disks. After the disk transfer, {project-short} automatically attempts to locate the already shared PVCs and the already migrated shared disks and attach them to the VMs. 
...
* In the second, set `migrateSharedDisks` to 'false` and migrate the following:
...
.Example of a correct division of shared disks, VMs, and unshared disks between the 2 Plan CRs
...
.Additional example of a correct division of shared disks, VMs, and unshared disks between the 2 Plan CRs
...
This migration "succeeds", but it results in a problem: Shared disk 2 is migrated twice by the first `Plan` CR. There are two workarounds for this problem. These are discussed in the Known issues section that follows the procedure.
