...
** All shared disks.
** For each shared disk, one of the VMs that is attached to it. If possible, choose VMs so that the plan does not contain any shared disks that are connected to more than one VM in the plan. See the figures below for further guidance.
...
This migration "succeeds", but it results in a problem: Shared disk 2 is migrated twice by the first `Plan` CR. This problem can be resolved by using one of the two workarounds that are discussed in the Known issues section which follows the procedure.
