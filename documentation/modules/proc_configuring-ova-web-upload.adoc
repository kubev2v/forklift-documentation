...
[role="_abstract"]
Configure Open Virtual Appliance (OVA) file upload to enable web browser uploads directly to an NFS share. Enable OVA management in the ForkliftController CR and then enable uploads for each provider in the {project-short} UI.
...
+
For information about creating a provider, see xref:proc_adding-source-provider_{context}[Adding an Open Virtual Appliance (OVA) source provider].
