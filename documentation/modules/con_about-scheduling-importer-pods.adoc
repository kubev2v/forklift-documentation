...
In {project-short} 2.11, scheduling importer pods is available only for migrations from the command-line interface. You schedule the importer pods in the `Plan` CR, as described in step 8 of xref:proc_migrating-vms-cli-vmware_vmware[Running a {vmw} vSphere migration from the command-line].
