...
[role="_abstract"]
Use a Red Hat {virt} provider as both source and destination. The automatically added host cluster serves both roles. Migrate VMs to or from the cluster where {project-short} is deployed.
...
[role="_abstract"]
Use a Red Hat {virt} provider as both source and destination. The automatically added host cluster serves both roles. You can add additional {virt} destination providers besides the default cluster where {project-short} is installed.
endif::[]
