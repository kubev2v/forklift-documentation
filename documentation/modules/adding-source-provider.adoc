...
In {project-short} versions 2.6 and later, you can also migrate VMs from standalone ESX/ESXi, or from ESX/ESXi servers that are managed by vCenter without going through vCenter.  This option is included in the procedure that follows.
...
* *Endpoint type*: Select the vSphere provider endpoint type. Options: *vCenter* or *ESXi*.
