...
+
. Optional, if you use a custom CA certificate or the system certificate: Fetch the CA certificate from the provider's API endpoint URL and confirm the validity of the certificate manually. Once confirmed, the CA certificate will be used to validate subsequent communication with the API endpoint.
+
...
.. To skip certificate validation, select the *Skip certificate validation* checkbox.
.. To validate a custom CA certificate, leave the *Skip certificate validation* checkbox cleared and _either_ drag the CA certificate to the text box _or_ browse for it and click *Select*.
endif::[]
ifdef::ova[]
