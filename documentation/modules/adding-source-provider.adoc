...
[IMPORTANT]
====
when you migrate an image-based VM that has an {osp} source provider, both the original VM and an identical copy of it appear in the destination provider, so you must delete one of them.
====
