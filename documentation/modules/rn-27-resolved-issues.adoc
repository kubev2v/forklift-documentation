...
In earlier releases of {project-short}, virtual machines (VM) were presenting XFS filesystem corruption after the warm migration from VMware to {virt} using {project-short}. This issue has been resolved in {project-short} 2.7.5. link:https://issues.redhat.com/browse/MTV-1679[(MTV-1679)]
...
In earlier releases of {project-short}, VMs could not be migrated successfully. This issue was caused by SUSE Linux using `/dev/disk/by-path` by default in their `/etc/fstab/` entry, which caused `virt-v2v` migrations to fail with errors as such: `virt-v2v: error: mount: mount_stub: /dev/disk/by-path/pci-0000:03:00.0-scsi-0:0:0:0-part3: No such file or directory`. This issue has been resolved in {project-short} 2.7.5. link:https://issues.redhat.com/browse/MTV-1660[(MTV-1660)]
...
In earlier releases of {project-short}, when migrating a virtual machine (VM) with *Secure Boot* enabled, the VM had *Secure Boot* as `disabled` after being migrated. This issue has been resolved in {project-short} 2.7.5. link:https://issues.redhat.com/browse/MTV-1632[(MTV-1632)]
