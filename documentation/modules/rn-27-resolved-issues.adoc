...
.Changing memory on VMs created by {project-short} does not correctly edit the VMs

In earlier releases of {project-short}, when changing the memory on virtual machines (VMs) created by {project-short}, the VMs were not correctly edited. This issue has been resolved in {project-short} 2.7.8. link:https://issues.redhat.com/browse/MTV-1760[(MTV-1760)]

.Unable to update `Migration Network` in VMware Provider from Hosts view

In earlier releases of {project-short}, it was impossible to update the `Migration Network` in the *VMware Provider* when using the *Hosts view*. This issue has been resolved in {project-short} 2.7.8. link:https://issues.redhat.com/browse/MTV-1682[(MTV-1682)]

// .VMs with `/dev/disk/by-path` cannot be migrated successfully
//
// In earlier releases of {project-short}, SUSE Linux uses `/dev/disk/by-path` by default in the `/etc/fstab/` entry, which means VMs could not be migrated successfully. This issue has been resolved in {project-short} 2.7.8. link:https://issues.redhat.com/browse/MTV-1660[(MTV-1660)]

.OVN secondary network is not working with Multus `default network` override

In earlier releases of {project-short}, the OVN (Open Virtual Network) secondary network does not function as expected with the Multus `default network` override. This issue has been resolved in {project-short} 2.7.8. link:https://issues.redhat.com/browse/MTV-1645[(MTV-1645)]

.`forklift-migration-precopy` snapshot remains on source after warm migration

In earlier releases of {project-short}, a `forklift-migration-precopy` snapshot remained on the source after warm migration. This issue has been resolved in {project-short} 2.7.8. link:https://issues.redhat.com/browse/MTV-1378[(MTV-1378)]
