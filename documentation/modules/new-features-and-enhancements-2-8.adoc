...
* VMware only: By default, {project-short} adds one or more randomly generated alphanumeric characters to the names of PVCs to ensure that all PVCs have unique names. In {project-short} 2.8.2, you can instruct {project-short} to not add such suffixes. However, if you choose this option, you might have PVCs whose names are not unique, which could cause a migration to fail.    