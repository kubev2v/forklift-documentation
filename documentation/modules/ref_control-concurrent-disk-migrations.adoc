...
{project-first} allows for concurrency to be controlled by using this variable; by default, it is set to 20.
...
In {project-short}, `MAX_VM_INFLIGHT` reserves one transfer slot per VM, regardless of current migration activity for a specific snapshot or the number of disks that belong to a single VM. The total set by `MAX_VM_INFLIGHT` is used to indicate how many concurrent VM transfers per ESXi host is allowed.
