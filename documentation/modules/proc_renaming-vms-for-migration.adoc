...
[role="_abstract"]
VM names must be DNS-compliant. You can rename source VMs in the {project-first} UI. If you do not specify target VM names, {project-short} automatically adjusts the original VM name to meet requirements for DNS-compliance. 
...
Automatic VM name transformation:: {project-full} automatically adjusts the name of a VM that does not comply with the rules. This feature allows a migration to proceed smoothly even if someone enters a VM name that does not follow the rules. {project-full} makes the following changes when it automatically generates a new VM name:
