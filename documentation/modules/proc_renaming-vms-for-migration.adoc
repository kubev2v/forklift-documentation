...
[role="_abstract"]
VM names must be DNS-compliant and must be unique in the {virt} environment for a migration to succeed. You can rename source VMs in the {project-first} UI. If you do not specify target VM names, {project-short} automatically adjusts the original VM name to meet requirements for DNS-compliance. 
...
If you do not specify a target VM name, {project-short} makes the following changes when it automatically adjusts the name of a VM:

** Removes characters that are not alphanumeric characters (a-z, 0-9) or hyphens (-)
...
** Enforces the 63 character limit
