...
** `.WinDriveLetter`: Windows drive letter in lowercase (for example, `c`, `d`). Requires the guest agent to be running on the source VM. Only applicable to Windows guests.
** `.FileName`: Name of the virtual disk file (for example, `vm-disk1.vmdk`). VMware only.
+
[NOTE]
====
The `.WinDriveLetter` variable requires the QEMU Guest Agent (or VMware Tools) to be active and reachable on the source VM. Additionally, `.WinDriveLetter` requires a disk key mapping between the guest OS and the hardware disk. This mapping may be missing for some disk configurations, particularly IDE disks. When the mapping is missing, {project-short} raises an information-level concern and the variable cannot be resolved in the PVC name template.

The `.FileName` variable provides only the filename (for example, `vm-disk1.vmdk`), not the full datastore path.
====
+
[IMPORTANT]
====
PVC names must comply with DNS-1123 naming rules: they must consist of lowercase alphanumeric characters or hyphens (`-`), start with an alphanumeric character, and not exceed 63 characters. {project-short} validates the `pvcNameTemplate` against the actual VM inventory when you create a migration plan. If the template syntax is invalid or if the resulting PVC names do not comply with Kubernetes naming rules, {project-short} adds an error to the plan conditions and prevents the migration from running.
====
+
*Template functions*
+
...
+
Functions can be chained together using the pipe (`|`) operator. For advanced use cases, regular expression functions such as `mustRegexReplaceAll` are also available.
+
Examples
