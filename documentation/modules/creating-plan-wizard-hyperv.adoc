...
[id="creating-plan-wizard-hyperv_{context}"]
= Creating a Microsoft Hyper-V migration plan by using the MTV wizard

[role="_abstract"]
You can migrate virtual machines from Microsoft Hyper-V by using the {project-full} plan creation wizard.

The wizard is designed to lead you step-by-step in creating a migration plan.

[NOTE]
====
{project-short} supports only cold migration from Microsoft Hyper-V. Virtual machines are shut down during the migration process.
====
