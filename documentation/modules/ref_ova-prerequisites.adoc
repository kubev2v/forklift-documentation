...
[id="ref_ova-prerequisites_{context}"]
...
[NOTE]
====
Migration of OVA files that were not created by {vmw} vSphere but are compatible with vSphere might succeed. However, migration of such files is not supported by {project-short}. {project-short} supports only OVA files created by {vmw} vSphere. Moreover, converting a vendor OVA may invalidate vendor support agreements.
