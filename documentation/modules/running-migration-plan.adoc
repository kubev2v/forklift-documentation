...
+
.. In the *Pods* section, in the *Pod links* column, click the *Logs* link. If the
+
The *Logs* tab opens.



Instead, we currently support viewing the logs via the UI per a VM migration in a plan by navigating to a plan -> 'virtual machines' tab -> expending a vm details -> going to the 'Pods' section  (if exists) -> clicking the 'pod logs' column.
