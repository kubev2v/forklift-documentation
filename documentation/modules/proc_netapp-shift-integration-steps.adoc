...
* You installed NetApp Shift according to the instructions on the NetApp site. 
...
+
Confirm that each PVC has the `vmID` label and the `forklift.konveyor.io/nfs-server` and `forklift.konveyor.io/nfs-path` annotations.
...
. Optional: Archive the migration plan after a successful migration. 
+
