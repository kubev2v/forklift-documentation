...
= Accessing logs and custom resource information by using the command-line interface

You can access logs and information about custom resources (CRs) by using the command-line interface (CLI) by using the `must-gather` tool. You must attach a `must-gather` data file to all customer cases.
