...
* *Limited support for dual-boot OS VMs:* {project-short} has limited support for the migration of dual-boot operating system VMs. In the case of a dual-boot OS VM, {project-short} attempts to convert the first boot disk it finds. Alternatively, you can specify the root device in the {project-short} UI.
