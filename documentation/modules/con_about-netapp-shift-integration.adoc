...
[role="_abstract"]
Use the NetApp Shift zero-copy integration with {project-first} to migrate {vmw} virtual machines (VMs) on NetApp NFS storage more efficiently. The NetApp Shift toolkit clones VM disks within the storage array in approximately one second. The toolkit also creates persistent volume claims (PVCs) on the target Red Hat {virt} cluster. The migration process then converts the guest operating system in place and creates the VM without transferring disk data over the network.
...
The NetApp Shift toolkit performs storage-side cloning and PVC creation. The {project-short} UI creates specially labeled and annotated PVCs that are displayed in the UI. These PVCs signal the NetApp Trident Container Storage Interface (CSI) driver to call internal NetApp tooling for the migration.

Phase 1 (storage) and phase 2 (migration) are separate:

* *Phase 1: NetApp Shift:* The NetApp Shift toolkit clones VM disks on the NFS array in approximately one second. It creates a PVC per disk in the target namespace. It also labels or annotates each PVC so the migration process can match it to the source VM. 
* *Phase 2: {project-short}:* You create and run a migration plan in the {project-short} UI. The migration process skips network disk transfer when matching PVCs exist. The process then converts disks with `virt-v2v-in-place` and creates the target VMs.

In a demonstration, over 1 TB of data was transferred in approximately one second by using this method.
...
|Performance
| Depends on array and network
| Over 1 TB in approximately 1 second (demo)
|===
