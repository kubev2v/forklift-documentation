...
EOF
----
<1> Optional for storage copy offload, required for other VMware migrations: Allowed values are `ReadWriteOnce` and `ReadWriteMany`.
...
EOF
----
<1> Allowed values are `ReadWriteOnce` and `ReadWriteMany`. Optional for storage copy offload migrations.
