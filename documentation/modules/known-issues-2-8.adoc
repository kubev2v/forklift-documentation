...
Linux guest VMs with clustered file systems on a shared disk are not supported as these disks are not added to guest the conversion. The `virt-v2v` tool does not update shared disks in the `fstab` configuration file. link:https://issues.redhat.com/browse/MTV-2145[(MTV-2145)]