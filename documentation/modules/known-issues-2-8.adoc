...
.Linux virtual machines with clustered file system are not supported
Linux guest virtual machines with clustered file systems on a shared disk are not supported. The virt-v2v may not update shared disks in the `fstab`configuration file. link:https://issues.redhat.com/browse/MTV-2145[(MTV-2145)]