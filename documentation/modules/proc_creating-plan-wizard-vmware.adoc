...
* Optional: If you are migrating disks encrypted using Linux Unified Key Setup (LUKS), you can create a `Secret` manifest with `type` set to `Opaque` that contains the decryption passphrase. You can then select this secret in the migration plan wizard instead of entering the passphrase manually.
