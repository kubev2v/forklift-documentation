...
* Have a {vmw} source provider and {a-virt} destination provider. For more information, see xref:proc_adding-source-provider_vmware[Adding a VMware vSphere source provider] or xref:proc_adding-virt-destination-provider_dest_vmware[Adding {a-virt} destination provider].
