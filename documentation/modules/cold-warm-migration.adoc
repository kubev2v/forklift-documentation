...
Live migration is available only for {virt}. It migrates running VMs from one {virt} cluster to another without a noticeable service interruption. The VMs maintain their active state throughout the process. Because live migration is relevant only for migration between {virt} clusters, it is discussed separately , rather than in the follow discussion of warm and cold migrations. 
// Add link. 
