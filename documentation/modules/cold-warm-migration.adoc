...
= Cold migration, warm migration, and live migration in {project-short}

[role="_abstract"]
{project-first} supports three types of migration, cold, warm, and live.
...
Live migration is available only for {virt}. Live migration migrates running VMs from one {virt} cluster to another without a noticeable service interruption. The VMs maintain their active state throughout the process. Because live migration is relevant only for migration between {virt} clusters, it is discussed separately, rather than in the following discussion of warm and cold migrations. 
// Add link. 
