...
----
+
The output, referred to here as `<openstack_token_output>`, includes the `token`, `userID`, and `projectID` that you need for authentication by using a token with user ID.


. Create a `Secret` manifest similar to the following:

** For authentication by using a token with user ID:
+
[source,yaml]
----
cat << EOF | oc apply -f -
...
** For authentication by using a token with user name:
+
[source,yaml]
----
cat << EOF | oc apply -f -
