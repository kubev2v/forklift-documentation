...
[id="con_virt-migration-workflow_{context}"]
...
. The `Migration Controller` service creates a `conversion` pod with the PVCs attached to it when importing from {vmw}.
+
...
*Cold migration from {rhv-full} or {osp} to the local {ocp-name} cluster:*
...
*Cold migration from {vmw} to the local {ocp-name} cluster:*
