...
[id="proc_selecting-migration-network-for-vmware-source-provider_{context}"]
...
* The target namespace must have network connectivity to the {vmw} source environment.
+
Migration pods run in the target namespace and require outbound access to the {vmw} API and ESXi hosts. If you use NetworkPolicies to restrict egress connections from the target namespace, you must configure policies that allow connections to {vmw}. This requirement applies whether you use the pod network, user-defined networks (UDNs), or cluster user-defined networks (CUDNs) in the target namespace.
