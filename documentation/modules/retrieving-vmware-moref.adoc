...
When you migrate VMs with a VMware vSphere source provider using {project-first} using the command-line interface (CLI), you need to know the managed object reference (moRef) of certain entities in vSphere, such as datastores, networks, and VMs.
