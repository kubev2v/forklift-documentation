...
+
ifdef::vmware[]
The value of the `id` key is the managed object reference for a {vmw} VM.
endif::[]
ifdef::rhv[]
The value of the `id` key is the universally unique identifier (UUID) for a {rhv-full} VM.
endif::[]
ifdef::ostack[]
The value of the `id` key is the universally unique identifier (UUID) for an {osp} VM.
endif::[]
ifdef::ova[]
The value of the `id` key is the VM name for an OVA VM.
endif::[]
ifdef::cnv[]
The value of the `id` key is the VM name for a {virt} VM.
endif::[]
ifdef::hyperv[]
The value of the `id` key is the globally unique identifier (GUID) for the Hyper-V VM.
endif::hyperv[]
