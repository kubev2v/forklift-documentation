...
[id="con_about-storage-maps_{context}"]
= About storage maps in migration plans

[role="_abstract"]
Create storage maps in {project-first} to map source disk storages to {virt} storage classes. You can create two types of storage map: maps for a specific migration plan and maps for use by any migration plan.
...
[NOTE]
====
When using storage copy offload, {project-short} supports migrating virtual machines with multiple disks (up to 60) across different storage arrays from the same vendor or different vendors. The system automatically maps each disk to the correct target datastore using Network Address Authority (NAA) identifier matching.
====
