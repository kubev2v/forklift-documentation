...
.The VM `Running` state is displayed during the migration
...
The {product-short} web console can no longer download logs. With this update, you must download every `must-gather` log image by using CLI commands. for more information, see link:https://github.com/openshift/must-gather-operator[Must Gather Operator].

.Migrated VMs can now be reviewed and configured

The {project-short} web console now displays the name and migration status of each virtual machine (VM). The migrated VMs have the `app: <vmname>` label to help further changes in {ocp-name} Virtualization. link:https://issues.redhat.com/browse/MTV-843[(MTV-843)]
...
.Validation for RHEL 6 guest operating system is no available
...
.Source and storage endpoints are now not selected automatically

Source network and source storage are now not selected automatically when creating a {ocp} to {ocp} plan. link:https://issues.redhat.com/browse/MTV-729[MTV729]

.Automatic retrieval of CA certificates for the provider's URL in the console

The ability to retrieve CA certificates, which was available in previous versions, has been restortes. The `vSphere Verify certificate` option is in the `add-provider` dialog. This option was removed in the transition to {ocp} console, and has now been added to the console. link:https://issues.redhat.com/browse/MTV-737[(MTV-737)]

.{ocp-name} Console SDK `fetch`, `post`, and `delete` calls are now available
...
With this update, a migration success status bar chart the number of migrations and displays their status - `starts`, `finished`, or `failed`. link:https://issues.redhat.com/browse/MTV-770[(MTV-770)]
...
.Removed credentials from ESXi provider host configuration

ESXi, as a vSphere provider, does not need credentials in its setup. This update removed the credentials requirements field from ESX settings. link:https://issues.redhat.com/browse/MTV-793[(MTV-793)]
...
.End-to-end test for imported OVA

The {project-short} process now performs an end-to-end test of the imported OVA. link:https://issues.redhat.com/browse/MTV-624[(MTV-624)]
...
Migration from OVA provider to a restricted project can now succeed past the `kubevirt` step. link:https://issues.redhat.com/browse/MTV-689[(MTV-689)]
...
.Migration of virtual machines with encrypted partitions fails during a conversion
...
.MTV Inventory now shows extended volume of the VM disk in RHV

The extended volume of a VM disk did not appear in the MTV Inventory for an RHV provider after it was already connected. The connected RHV provider did not update the volume of the VM disk. This issue has been resolved in {project-short} 2.6.0. link:https://issues.redhat.com/browse/MTV-829[(MTV-829)]
