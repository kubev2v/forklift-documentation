...
The {product-short} web console can no longer download logs from the Must Gather operator. Every `mist-gather` log image download must be preformed using CLI commands. See link:https://github.com/openshift/must-gather-operator[Must Gather Operator] for more information.
