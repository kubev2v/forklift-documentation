...
When migrating a VM with NVME disks from vSphere, the migration process fails and the Web Cosole shows that the `Convert image to kubvirt` stage is 'running` but did not finish successfully. link:https://issues.redhat.com/browse/MTV-963([MTV-963])
