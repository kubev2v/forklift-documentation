...
Previously, if a VM was configured with a disk that was on a datastore that was no longer available in vSphere at the time a migration was attempted, the `forklift-controller` crashed, rendering {project-short} unusable. In {project-short} 2.6.1, {project-short} presents a critical validation for VMs with such disks, informing users of the problem, and the `forklift-controller` no longer crashes, although it cannot transfer the disk. link:https://issues.redhat.com/browse/MTV-1029[(MTV-1029)]
