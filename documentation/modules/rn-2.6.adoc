...
In earlier releases of {project-short}, during the migration of Rocky 8, CentOS 7.2+, and Ubuntu 22 virtual machines (VM) from VMware to {ocp} (OCP) version 4.13, the name of the network interfaces is modified, and the static IP configuration for the VM is no longer functional. This issue has been resolved for static IPs in {project-short} 2.6.5. link:https://issues.redhat.com/browse/MTV-595[(MTV-595)]
...
.Migrating RHEL and CentOS 7.0 and 7.1 from VMware to {ocp}, the name of the network interfaces changes

When migrating a simple Red Hat Enterprise Linux (RHEL) 7.0 and 7.1, as well as Rocky Linux and CentOS 7.0 and 7.1, virtual machine (VM) from VMware 7 to {ocp} 4.13, the name of the network interfaces changes and the static IP configuration for the VM no longer functions. This issue is caused by RHEL 7.0 and 7.1 still requiring `virtio-transitional`. Workaround: Manually update the guest to RHEL 7.2 or update the VM specification post-migration to use transitional. link:https://issues.redhat.com/browse/MTV-1382[(MTV-1382)]
