...
. In the row of the appropriate source provider, click the *VMs* entry.
. Select the VMs you want to migrate and click *Create migration plan*.
+
The *Create migration plan* wizard opens. It displays the source provider's name and suggestions for a plan name, a target provider and namespace, a network map, and a storage map.
