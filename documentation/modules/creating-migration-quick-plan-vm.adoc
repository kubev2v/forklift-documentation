...
* *Preserving the CPU type of VMs that are migrated from {rhv-short}*: By default, {project-short} sets the CPU type (CPU model and any flags) of migrated VMs if the type is not set specifically for the VM. This can be an issue for migrations from {rhv-short} because the CPU type of {rhv-short} VMs can be set at the cluster level, in which case {project-short} might change the type.
+
To preserve the CPU type of your {rhv-short} VMs, in the *Settings* section, click the Edit icon next to *Preserve CPU model*. toggle the *Whether to preserve the CPU model* switch, and then click *Save*.
