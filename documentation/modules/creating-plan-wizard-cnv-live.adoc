...
As described in the specific prerequisites for live migration between {virt} clusters.
//Add link.
...
* *Source provider*: Select from the list. You can use any {virt} provider. There is no need to create a new one for a live migration. 
...
. Click *Next*.
+
. On the *Virtual machines* page, select the virtual machines you want to migrate and be sure they are powered on. A live migration will fail if any of its VMs are powered off.
+
. Click *Next*.
+
...
+
 ...
 +
 If they are both {virt} clusters, have someone with `admin-cluster` privileges ensure that the CNV-CNV prerequisites are met.
// Add link.

. Click *Next*.
+
. On the *Other settings (optional)* page, you have the option to change the *Transfer network* of your migration plan. However, you can ignore this page. 
