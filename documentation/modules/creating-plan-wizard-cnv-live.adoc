...
+
If they are both {virt} clusters, have someone with `admin-cluster` privileges ensure that the prerequisites for live migration between {virt} clusters are met. For more information, see xref:cnv-cnv-live-prerequisites_mtv[{virt} live migration prerequisites].
