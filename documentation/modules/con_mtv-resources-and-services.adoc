...
[IMPORTANT]
====
For successful migrations, {project-short} automatically deletes PVC mounts, conversion pods, and populator pods and changes the plan's status to *Complete* in the UI, or `Succeeded` in the YAML file. For failed migrations, the plan's status is *Incomplete* in the UI, or `failed` in the YAML file, and PVC mounts are kept so you can investigate the failure. You must archive a failed migration plan.
====
