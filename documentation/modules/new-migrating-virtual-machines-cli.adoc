...
spec:
  ...
  map: <3>
    ...
    storage: <6>
      ...
      namespace: <namespace>
  preserveStaticIPs: <8>
  networkNameTemplate: <"net-{{.NetworkIndex}}"> <9>
  pvcNameTemplate: <"{{.VmName}}-disk-{{.DiskIndex}}"> <10>
  targetNamespace: <target_namespace>
  volumeNameTemplate: <"disk-{{.VolumeIndex}}"> <11>
  vms: <12>
    - id: <source_vm> <13>
      name: <source_vm> 
      networkNameTemplate:   "{{if eq .NetworkType "pod"}}pod{{else}}multus-{{.NetworkIndex}}{{end}}" <14>
      pvcNameTemplate:   "{{.VmName}}-disk-{{.DiskIndex}}" <15> 
      volumeNameTemplate:   "pvc-{{.PVCName}}" <16>
      hooks: <17>
        - hook:
            namespace: <namespace>
            name: <hook> <18>
...
<9> Generates a network interface name for a target VM that overrides the template set in the plan.
...
+
Variable names cannot exceed 63 characters. This rule applies to a network name network template, a PVC name template, a VM name template, and to a volume name template. 
<10> Generates a persistent volume claim (PVC) name for a VM disk.
The template follows the Go template syntax and has access to the following variables:
...
+
<11> Generates a volume interface name for a target VM.
The template follows the Go template syntax and has access to the following variables:
