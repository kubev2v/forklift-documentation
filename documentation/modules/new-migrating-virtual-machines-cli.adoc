...
<5> Enter the CA certificate for the server that serves the SDK, either vCenter or ESXi, depending on the configuration of the provider.
...
<6> Enter the CA certificate for connecting to the source environment. Note that if you set `insecureSkipVerify` to `false` and you do not set  `cacert`, {project-short} attempts to use the system CA.
<7> Specify the API endpoint URL, for example, `https://<identity_service>/v3`.
endif::[]
