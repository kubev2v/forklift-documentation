...
----
<1> Allowed values are `pod` and `multus`.
<2> Specify a network attachment definition for each additional {virt} network. Specify the `namespace` either by using the `namespace property` or with a name as follows: `<network_namespace>/<network_name>`.
...
spec:
  ...
  volumeNameTemplate: <volume_name_template> <13>
  vms: <14>
    - id: <source_vm1> <15>
    - name: <source_vm2>
      networkNameTemplate: <network_interface_template_for_this_vm> <16>
      pvcNameTemplate: <pvc_name_template_for_this_vm> <17>
      volumeNameTemplate: <volume_name_template_for_this_vm> <18>
      targetName: <target_name> <19>
      hooks: <20>
        - hook:
            namespace: <namespace>
            name: <hook> <21>
...
+
<12> Determines whether VMs are converted before migration using a tool named `virt-v2v` that makes the VMs compatible with {virt}.
...
+
<13> [[callout13]]Optional: Specify a template for the volume interface name for the VMs in your plan.
The template follows the Go template syntax and has access to the following variables:
...
** `"disk-{{.VolumeIndex}}"`
** `"pvc-{{.PVCName}}"`
<14> You can use either the `id` or the `name` parameter to specify the source VMs.
<15> Specify the VMware vSphere VM moRef. To retrieve the moRef, see xref:retrieving-vmware-moref_vmware[Retrieving a VMware vSphere moRef].
<16> Optional: Specify a network interface name for the specific VM. Overrides the value set in `spec:networkNameTemplate`. Variables and examples as in xref:callout9[callout 9].
<17> Optional: Specify a PVC name for the specific VM. Overrides the value set in `spec:pvcNameTemplate`. Variables and examples as in xref:callout10[callout 10].
<18> Optional: Specify a volume name for the specific VM. Overrides the value set in `spec:volumeNameTemplate`. Variables and examples as in xref:callout13[callout 13].
<19> Optional: {project-short} automatically generates a name for the target VM. You can override this name by using this parameter and entering a new name. The name you enter must be unique and it must be a valid Kubernetes subdomain. Otherwise, the migration fails automatically.
<20> Optional: Specify up to two hooks for a VM. Each hook must run during a separate migration step.
<21> Specify the name of the `Hook` CR.
<22> Allowed values are `PreHook`, before the migration plan starts, or `PostHook`, after the migration is complete.
+
