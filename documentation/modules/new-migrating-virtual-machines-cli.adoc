...
<2> Specify the VMware vSphere datastore moRef. For example, `f2737930-b567-451a-9ceb-2887f6207009`. To retrieve the moRef, see xref:retrieving-vmware-moref_vmware[Retrieving a VMware vSphere moRef].name:
endif::[]
...
spec:
  ...
  map: <3>
    ...
    storage: <6>
      name: <storage_map> <7>
      namespace: <namespace>
  networkNameTemplate: <"net-{{.NetworkIndex}}"> <8>
  preserveStaticIPs: <9>
  pvcNameTemplate: <"{{.VmName}}-disk-{{.DiskIndex}}"> <10>
  targetNamespace: <target_namespace>
  vms:
    - id: <source_vm> <11>
      name: <source_vm> <12>
      networkNameTemplate:   "{{if eq .NetworkType "pod"}}pod{{else}}multus-{{.NetworkIndex}}{{end}}" <13>
      pvcNameTemplate:   "{{.VmName}}-disk-{{.DiskIndex}}" <14> 
      volumeNameTemplate:   "pvc-{{.PVCName}}" <15>
      hooks: <16>
        - hook:
            namespace: <namespace>
            name: <hook> <17>
          step: <step> <18>
  volumeNameTemplate: <"disk-{{.VolumeIndex}}"> <19>
EOF
----
...
<8> Generates a network interface name for a target VM that overrides the template set in the plan.
...
+
Variable names cannot exceed 63 characters. This rules to a network name network templat, a PVC name template, a VM name template, and to a volume name template. 
<9> By default, virtual network interface controllers (vNICs) change during the migration process. As a result, vNICs that are configured with a static IP address linked to the interface name in the guest VM lose their IP address.
...
<10> Generates a persistent volume claim (PVC) name for a VM disk that overrides the template set in the plan.
The template follows the Go template syntax and has access to the following variables:
...
<11> Specify the VMware vSphere VM moRef. To retrieve the moRef, see xref:retrieving-vmware-moref_vmware[Retrieving a VMware vSphere moRef].
<12> Optional.
<13> As stated in callout 8: +
The template follows the Go template syntax and has access to the following variables:
* `.NetworkName:` If the target network is `multus`, add the name of the Multus Network Attachment Definition. Otherwise, leave this variable empty.
* `.NetworkNamespace`: If the target network is `multus`, add the namespace where the Multus Network Attachment Definition is located.
...
+
Variable names cannot exceed 63 characters.
<15> Generates a volume interface name for a target VM that overrides the template set in the plan.
The template follows the Go template syntax and has access to the following variables:
...
+
Variable names cannot exceed 63 characters
<16> Optional: You can specify up to two hooks for a VM. Each hook must run during a separate migration step.
<17> Specify the name of the `Hook` CR.
<18> Allowed values are `PreHook`, before the migration plan starts, or `PostHook`, after the migration is complete.
<19> As stated in callout 15:
** `.PVCName`: Name of the PVC mounted to the VM using this volume
** `.VolumeIndex`: Sequential index of the volume interface (0-based)
+
Variable names cannot exceed 63 characters. 
...
<9> Specify the {rhv-short} VM UUID.
<10> Optional.
