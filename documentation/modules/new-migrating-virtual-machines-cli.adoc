...
// Add an inline anchor, which will use in an xref
<2> [[anchor-point]]Specify the name of provider `Secret` CR.
endif::[]
...
[NOTE]
====
* If the migrated machines are set with a custom CPU model, they will be set with that CPU model in the destination cluster, not matter what the setting of `preserveClusterCpuModel` is.
