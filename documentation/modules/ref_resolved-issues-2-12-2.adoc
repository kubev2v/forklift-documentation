...
Before this update, SCSI reservations were enabled by default on all shared Raw Device Mapping (RDM) disks during migration. As a consequence, issues occurred when a reservation was not desired or supported for a given workload. With this update, a new plan-level setting that applies to all virtual machines (VMs) in a migration plan and a per-VM override are available. As a result, you have explicit control over whether SCSI reservations are enabled for shared RDM disks, which prevents unintended reservation behavior and allows fine-grained configuration per workload.
+
