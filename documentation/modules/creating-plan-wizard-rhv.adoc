...
[id="creating-plan-wizard-rhv_{context}"]
...
* If you are using a user-defined network (UDN), note the name of the namespace as defined in {virt}.
...
* *Target project*: Select from the list or click *Create Project*. If you are using a UDN, use the namespace defined in {virt}.

. If you clicked *Create Project*, specify the following fields and then click *Create Project*:

* *Name*: Enter the name of the project. A project name must consist of lowercase alphanumeric characters or `-`. A project name must start and end with alphanumeric characters. Examples: `my-name` or `123-abc`.
* Optional: *Display name*: Enter the display name for the project.
* Optional: *Description*: Enter a description of the project.
. Click *Next*.
+
...
* *Preserve CPU mode*: Generally, the CPU model (type) for {rhv-full} VMs is set at the cluster level. However, the CPU model can be set at the VM level, which is called a custom CPU model.
+
...
+
To preserve the cluster-level CPU model of your {rhv-full} VMs, do the following:
