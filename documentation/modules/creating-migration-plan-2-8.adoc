...
* *Shared disks*: Applies to cold migrations only. Shared disks are disks that are attached to multiple VMs and that use the multi-writer option. As a result of these characteristics, shared disks are difficult to migrate.By default, {project-short} does not migrates shared disks. 
+
** To migrate shared disks in the migration plan, click the *Edit* icon, toggle the *Migrate shared disks* switch, and click *Save*.
+
Note that migrating shared disks might slow down the migration process. 

* *PVC name template*: The PVC name template is used to generate a persistent volume claim (PVC) name for a VM disk.
You can use this setting to override the template set in the plan.
+
...
+
Names can not exceed 63 characters, so keep variable names short. 
+
** To specify a different PVC name template, do the following:

*** Click the *Edit* icon.
*** Click *Enter custom namimg template*.
*** Enter the name of the template.
*** Click *Save*.

* *Volume name template*: The volume name template is used to generate a volume interface names in a target virtual machine. You can use this setting to override the template set in the plan.
+
The template follows the Go template syntax and has access to the following variables:
...
Names can not exceed 63 characters, so keep variable names short. 
+
** To specify a different volume name template, do the following:
...
* *Network name template*: The network name template is used to generate a volume interface names in a target virtual machine. You can use this setting to override the template set in the plan.
+
The template follows the Go template syntax and has access to the following variables:

** `.NetworkName:` If the target network is `multus`, use the name of the Multus Network Attachment Definition. Otherwise, leave empty.
** `.NetworkNamespace`: If the target network is `multus`, use the namespace where theMultus Network Attachment Definition is located.
...
Names can not exceed 63 characters, so keep variable names short. 
+
** To specify a different network name template, do the following:
...
>>>>>>> 873f604e (draft of shared disk description)
