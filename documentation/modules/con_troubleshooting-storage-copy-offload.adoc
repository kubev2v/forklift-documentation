...
** Disabled SSH
+
*Solution*: Manually enable an SSH connection on the ESXi host by using the commands in "Setting up storage copy offload by using manually generated SSH keys" in _Planning your virtual machines to Red Hat OpenShift Virtualization_. A link for these instructions is available in the "Additional resources" section at the end of this module.
...
. Set a management interface for the SVM and enter its `hostname` in the `STORAGE_HOSTNAME` parameter by following the instructions in the NetApp Knowledge Base article, "Trident fails to access ONTAP on SVM level and on Cluster level." A link for these instructions is available in the "Additional resources" section at the end of this module. The link requires you to log in.

[role="_additional-resources"]
.Additional resources
* link:{mtv-mig}assembly_planning-migration-vmware_mtv#proc_storage-copy-offload-manual-ssh-set-up_vmware[Setting up storage copy offload using manually generated SSH keys]
* link:https://kb.netapp.com/Cloud/Astra/Trident/Trident_fails_to_access_ONTAP_on_SVM_level_and_on_Cluster_level[Trident fails to access ONTAP on SVM level and on Cluster level]
