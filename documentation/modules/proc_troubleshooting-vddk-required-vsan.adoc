...
[role="_abstract"]
Virtual machine migrations fail with an error when a VM is backed by {vmw} vSAN storage and no {vmw} Virtual Disk Development Kit (VDDK) image is configured.

VDDK is mandatory for migrations from {vmw} vSAN storage. Migrations do not work without VDDK when a VM is backed by vSAN.
