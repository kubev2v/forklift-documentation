...
{The} {project-first} enables you to migrate a large number of virtual machines from VMware vSphere to {virt} running on {ocp} {ocp-virt}. You can perform the migration by using the {ocp} web console or the command line interface (CLI).
...
You can migrate virtual machines with the {ocp} web console or from the CLI.
