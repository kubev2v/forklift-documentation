...
*  VMware only: You must be logged in as a user with the  xref:vmware-privileges_{context}[VMware privileges].
====
...
[IMPORTANT]
====
*  You must be logged in as a user with `cluster-admin` privileges. VMware only: The xref:vmware-privileges_{context}[VMware privileges] are sufficient.
