...
[IMPORTANT]
====
The administrator must ensure that no data consistency issues arise when a guest initiates storage connections independently, such as Internet Small Computer Systems Interface (iSCSI) connections or Network File System (NFS) mounts.
