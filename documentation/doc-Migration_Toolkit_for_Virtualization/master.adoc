...
{product-first} has software requirements for all providers as well as specific software requirements per provider.
...
You can migrate virtual machines (VMs) by using the {project-short} user interface, which is located in the *Virtualization* section of the {ocp} web console.
...
==== Migrating virtual machines from VMvSphere
...
==== Migrating virtual machines from OStack
