...
== Migrating virtual machines using the command-line interface

You can migrate virtual machines to {virt} using the command-line interface (CLI).
...
You can add hooks a migration plan using the command-line interface (CLI) by using the {project-full} API.
