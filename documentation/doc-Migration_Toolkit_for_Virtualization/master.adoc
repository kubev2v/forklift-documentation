...
* The _step-by-step_ method in which you create plans components - such as providers and network mappings - and then, combine them into a migration plan. Although do not have to create the components in a definite order, the steps are listed below in the recommended order of work.
* The _quick plan_ method in which you combine previously-defined components into a migration plan. This method is described in its own section, which follows the procedures for all of the steps in creating a migration plan.
