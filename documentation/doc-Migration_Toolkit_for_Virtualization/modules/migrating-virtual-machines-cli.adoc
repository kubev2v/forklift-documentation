...
. Obtain the SHA-1 fingerprint of the ESXi host:
+
[source,terminal]
----
$ openssl s_client \
...
EOF
----
<1> Specify the ESXi administrator account, for example, `administrator@vsphere.local`.
<2> Specify the ESXi password.
<3> Specify the SHA-1 fingerprint of the ESXi host.
